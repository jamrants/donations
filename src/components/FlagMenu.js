  ...
  return locales.map(locale => {
    return (
      <MenuItem py="4px" width="fit-content" onClick={() => onClick(locale)}>
        <ReactCountryFlag style={{ height: "36px", width: "36px" }} countryCode={locale.Code} svg />
      </MenuItem>
    )
  })
}
...
const FlagMenu = ({ onClick, locales, activeLocale }) => {
  return (
    <Menu>
      <MenuButton lineHeight="initial" pb="2px" px="4px" ml="4px" borderBottom="2.5px solid #F9FAFC">
        <ReactCountryFlag style={{ height: "36px", width: "36px" }} countryCode={activeLocale.Code} svg />
